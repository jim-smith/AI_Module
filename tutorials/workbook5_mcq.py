...
Q5 = create_multipleChoice_widget('What is the effect on KMeans if one feature has much larger range of values than the others?', [ 'None', 'That feature dominates the clustering', 'That feature is ignored'], 'That feature dominates the clustering')

Q6 = create_multipleChoice_widget('What is  effect does the MinMaxScale() have on each feature (column)', [ 'None', 'It scales it evenly to the range [0,1]', 'It compresses high and low values','It changes it to mean 0 and std. deviation 1', 'It scales the data to the range [-1,1]'], 'It scales it evenly to the range [0,1]')