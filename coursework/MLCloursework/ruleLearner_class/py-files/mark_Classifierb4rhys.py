...
def MarkClassifier(): 
    ...
    for setNum in range(len(datasets)):
        ...
        studentModel = Correct(maxRules=5,increments=100) 
        ...
        message = message + "On dataset {}, {}, your method got a test accuracy of {}<br>".format(setNum,setDesc[setNum],studentTestScore)
        message = message +"\tThe target for this dataset was {} so on this dataset the difference was {}<br>".format(correctTestScore,correctDeltas[setNum])

        # feedback about their predict() method from testing the student's learned model with my code
        if(studentNumRules>0):
            message = message + "\tTesting the set of {} rules learned by your code using a different implementation of predict()<br>".format(studentNumRules)
            ...
            if(np.array_equal(st_ypred, corr_ypred)):
                message = message + "\tThis gave the same results, indicating that  your predict() method is probably correct.<br>"
            else:
                 message = message + "\tThis gave a different results indicating that your predict() method is not correct.<br>"
        else:# (studentNumRules==0)
            madeNoRules += 1
            message = message + "\tHowever, your algorithm had a value of 0 for self.numRules, and getRuleSet() returned an empty array.<br>"
            message = message + "\tSo we could not test your predict() method for this dataset.\n"

        
        if(len(studentInvalids)>0):
            message = message +"\tCrucially, you algorithm predicted labels that were not present in the training set. This should be impossible.<br>"
            madeInvalid +=1
        message = message + "<br>"
    ...
    message = message+"Overall:<br>"
    finalScore = 0
    if (madeInvalid>0):
        message = message + "Your algorithm made some invalid predictions, therefore you score 0.<br>"
        finalScore = 0
    else:
        wrongAlg = False
        ...
        if(madeNoRules ==len(datasets)):
            ...
            message = message + " This  suggests you have implemented a different algorithm or not used the super class correctly.<br>"
            wrongAlg = True
        if(np.mean(knnDeltas) < 2):
            message = message + "The results are very close to what a 1-NN classifier gets. Did you implement this by mistake?<br>"

        meanDiff= np.mean(correctDeltas)
        maxDiff = np.max(correctDeltas)
        message = message + "Your algorithm makes valid predictions."
        if(maxDiff>=10):
            message = message + " However,  the test accuracy more than 10 percent away from the target on one or more datasets, so you score 20.<br>"
            finalScore = 20
        elif ( meanDiff <10 and meanDiff>5):
            message = message + "The mean difference to the target test accuracy is between 5 and 10 percent, so you score 40.<br>"
            finalScore=40
            ...
            message = message + " However, the differnce is greater than 5 for one or more datasets so you score 50.<br>"
            finalScore = 50
        elif ( (meanDiff <= 5.0)and (meanDiff >2.5) and (maxdiff <5)):
            message = message + "On average your  test accuracy is between 2.5 and 5 percent away from the target."
            message = message + " The differnce is less than 5 for all datasets so you score 60.<br>"
            finalScore = 60
        elif(meanDiff >= 1.0 and meanDiff<2.5):
            message =message + "On average your  test accuracy is within 1 and 2.5 percent of the target accuracy so you score 70.<br>"
            finalScore = 70
        elif(meanDiff < 1.0):
            message = message + "The mean test accuracy is less than 1 percent away from target. You score 100 for this run.<br>"
            finalScore = 100
