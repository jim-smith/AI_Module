    ...
    def fit( self,train_X,train_y):
       # store the set of different labels - don't assume theyare 0,12,2 etc
        ...
        while (self.score<numTrItems and self.numRules<self.maxRules):
            ...
            ## Faulty V1: loop conditions are incorrect
            featMax = self.numFeatures 
            opMax=3
            threshMax= self.numThresholds
            labelMax = len(self.labels)
            V1fault = False
            if(V1fault):
                featMax += -1
                threshMax += -1
            ...
            for feature in range(featMax):
                for operator in range (opMax):
                    for threshold in range (threshMax):
                        for label in range(labelMax):
